...
def applyOp (op: Op) (data: Vector Nat n): Nat :=
  let (o, x): (Nat -> Nat -> Nat) × Nat := match op with
  | Op.Mult => ((· * ·), 1)
  | Op.Add => ((· + ·), 0)
  data.toList.foldl o x
...
def main : IO Unit := do
  ...
  if let opline::datalines := lines.reverse then
    ...
    let rows := datalines.map spaceSplit |>.map (·.map String.toNat! |>.toArray) |>.toArray
    let grid := Grid.ofRows rows |>.data |> transpose
    let results := ops.zipWith applyOp grid.toList 
